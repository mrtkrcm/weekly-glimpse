...
import { tasks, user, session, googleAccounts, googleCalendars, sharedCalendars, calendarMembers } from '../db/schema';
...
const taskInsertSchema = createInsertSchema(tasks, {
  ...
  dueDate: (schema) => schema.openapi({
    description: 'When the task is due',
    example: '2024-03-20T10:00:00Z'
  }),
  priority: (schema) => schema.openapi({
    description: 'The priority level of the task',
    example: 'high'
  }),
  status: (schema) => schema.openapi({
    description: 'The current status of the task',
    example: 'in_progress'
  })
}).omit({ id: true, createdAt: true, updatedAt: true }).openapi('TaskCreate');
...
}).omit({ id: true, createdAt: true, updatedAt: true }).openapi('CalendarCreate');
...
const calendarMemberInsertSchema = createInsertSchema(calendarMembers, {
  role: (schema) => schema.openapi({
    description: 'The role of the member in the calendar',
    example: 'editor'
  })
}).omit({ id: true, createdAt: true, updatedAt: true }).openapi('CalendarMemberCreate');
...
const paginatedResponseSchema = <T extends z.ZodType>(schema: T) => z.object({
  data: z.array(schema),
  pagination: z.object({
    page: z.number(),
    limit: z.number(),
...
registry.registerPath({
  ...
  request: {
    query: z.object({
      status: z.enum([TASK_STATUS.TODO, TASK_STATUS.IN_PROGRESS, TASK_STATUS.DONE]).optional().openapi({
        description: 'Filter tasks by status',
        example: TASK_STATUS.IN_PROGRESS
      }),
      priority: z.enum([TASK_PRIORITY.LOW, TASK_PRIORITY.MEDIUM, TASK_PRIORITY.HIGH]).optional().openapi({
